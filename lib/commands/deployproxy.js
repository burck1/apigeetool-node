...
module.exports.run = function(opts, cb) {
  ...
  async.series(
    ...
    function(err, results) {
      ...
      async.map(
        _.values(results[results.length - 1]),
        function(result, cb) {
          if (opts.debug) {
...
function checkBasePath(opts, cb) {
  ...
  findProxies(opts, function(err, files) {
    if (files.length !== 1) {
      return cb(
        new Error(
          'Cannot specify base-path option when more than one ProxyEndpoint'
        )
      );
    }
    fs.readFile(files[0], 'utf8', function(err, data) {
      ...
      if (match && match[1] !== opts['base-path']) {
        return cb(
          new Error(
            util.format('Cannot override ProxyEndpoint BasePath: %s', match[1])
          )
        );
      }
      cb();
    ...
    function(err, req, body) {
      if (err) {
        ...
        opts.deploymentVersion =
          parseInt(
            _.max(body.revision, function(r) {
              return parseInt(r);
            })
          ) + 1;
        if (opts.verbose) {
...
function createApiProxy(opts, request, done) {
  ...
  var uri = util.format(
    '%s/v1/o/%s/apis?action=import&validate=false&name=%s',
    opts.baseuri,
    opts.organization,
    opts.api
  );
...
function proxyCreationDone(err, req, body, opts, done) {
  if (err) {
    ...
    if (opts.verbose) {
      ...
      new Error(
        util.format('Proxy creation failed. Status code %d', req.statusCode)
      )
    );
  }
}
...
function uploadResources(opts, request, done) {
  ...
  async.eachLimit(
    ...
    function(entry, entryDone) {
      var uri = util.format(
        '%s/v1/o/%s/apis/%s/revisions/%d/resources?type=%s&name=%s',
        opts.baseuri,
        opts.organization,
        opts.api,
        opts.deploymentVersion,
      ...
      if (
        opts['bundled-dependencies'] &&
        (entry.resourceType === 'node' || entry.resourceType === 'hosted')
      ) {
        // skip node/hosted files we will handle these special
        return entryDone();
      } else if (entry.directory) {
        // ZIP up all directories, possibly with additional file prefixes
        ziputils.zipDirectory(entry.fileName, entry.zipEntryName, function(
          err,
          zipBuf
        ) {
          if (err) {
...
function uploadPolicies(opts, request, done) {
  ...
  async.eachLimit(
    ...
    function(fileName, itemDone) {
      ...
      var uri = util.format(
        '%s/v1/o/%s/apis/%s/revisions/%d/policies',
        opts.baseuri,
        opts.organization,
        opts.api,
        opts.deploymentVersion
      ...
      var postReq = request(
        ...
        function(err, req, body) {
          if (err) {
            ...
            itemDone(
              new Error(
                util.format('Error uploading policy %s: %s', fileName, body)
              )
            );
          }
        }
...
function uploadTargets(opts, request, done) {
  ...
  async.eachLimit(
    ...
    function(fileName, itemDone) {
      ...
      var uri = util.format(
        '%s/v1/o/%s/apis/%s/revisions/%d/targets?name=%s',
        opts.baseuri,
        opts.organization,
        opts.api,
        opts.deploymentVersion,
      ...
      var postReq = request(
        ...
        function(err, req, body) {
          if (err) {
            ...
            itemDone(
              new Error(
                util.format('Error uploading target: %s', req.statusCode)
              )
            );
          }
        }
...
function uploadProxies(opts, request, done) {
  findProxies(opts, function(err, filePaths) {
    ...
    async.eachLimit(
      filePaths,
      opts.asynclimit,
      function(filePath, itemDone) {
        var proxyName = filePath
          .split(path.sep)
          .pop()
          .split('.')[0];
        if (opts.verbose) {
        ...
        var uri = util.format(
          '%s/v1/o/%s/apis/%s/revisions/%d/proxies?name=%s',
          opts.baseuri,
          opts.organization,
          opts.api,
          opts.deploymentVersion,
...
function runNpm(opts, request, done) {
  if (!opts.remoteNpm && (!opts['bundled-dependencies'] || noNodeSource)) {
    ...
    request(
      {
        uri: util.format(
          '%s/v1/o/%s/apis/%s/revisions/%d/npm',
          opts.baseuri,
          opts.organization,
          opts.api,
          opts.deploymentVersion
...
function deployProxy(opts, request, done) {
  ...
  function deployToEnvironment(environment, done) {
    var uri = util.format(
      '%s/v1/o/%s/e/%s/apis/%s/revisions/%d/deployments',
      opts.baseuri,
      opts.organization,
      environment,
      opts.api,
  ...
  if (opts.verbose) {
    console.log(
      'Delaying deployment for %d seconds',
      opts.waitAfterImportDelay
    );
  }
  setTimeout(function() {
